...
impl From<u32> for PowerOnReason {
    fn from(value: u32) -> Self {
        ...
        match masked_value {
            Self::COLD_PWR_ON => PowerOnReason::ColdPwrOn,
            Self::SYS_RESET_PIN => PowerOnReason::SysResetPin,
            Self::BY_PPONR => PowerOnReason::ByPponr,
            Self::BY_GIC_WAKE_REQ_SIGNAL => PowerOnReason::ByGicWakeRequestSignal,
            _ => unreachable!(),
        }
    }
}
...
impl<'a> FvpPowerController<'a> {
    ...
    /// Provides the value of the PSYS register as a u32.
    /// If only the Power on Reason is needed, please see `power_on_reason`.
    /// If only the rest of the contents of the register are necessary, please see `system_status`
    pub fn system_status_reg(&mut self, mpidr: u32) -> u32 {
